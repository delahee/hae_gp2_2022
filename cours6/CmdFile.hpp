...
public:
	...
	static bool saveScript(std::string path, std::vector<Cmd>& data) {
		...
		for (auto& cmd : data) {
			switch (cmd.id) {
			case CmdId::Advance:		fprintf(f, "Advance %lf\n",  cmd.data); break;
			case CmdId::RotateLeft:		fprintf(f, "RotateLeft %lf\n", cmd.data); break;
			case CmdId::RotateRight:	fprintf(f, "RotateRight %lf\n", cmd.data); break;
